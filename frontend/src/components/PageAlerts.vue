<template>
  <div class="flex flex-col gap-6">
    <div class="flex items-start justify-between flex-wrap gap-3">
      <div>
        <h1 class="font-syne text-2xl font-extrabold text-ink">Alert Early Warning System</h1>
        <p class="text-sm text-ink-3 mt-0.5">Notifikasi siswa yang memerlukan perhatian segera</p>
      </div>
      <button class="btn btn-secondary" @click="$emit('toast', '✓ Semua alert ditandai dibaca.')">
        ✓ Tandai Semua Dibaca
      </button>
    </div>

    <!-- Kritis -->
    <div class="card">
      <div class="card-title">Kritis — Intervensi Segera</div>
      <div class="card-sub">Siswa dengan skor brain rot sangat tinggi</div>
      <div v-if="!kritis.length" class="text-center py-6">
        <div class="text-3xl mb-2">✅</div>
        <div class="text-sm font-semibold text-ink-2">Tidak ada siswa kritis</div>
      </div>
      <div v-else class="flex flex-col gap-2.5">
        <div v-for="s in kritis" :key="s.nama"
          class="flex items-start gap-3.5 p-4 rounded-lg border bg-danger-bg border-danger-mid">
          <div class="text-lg mt-0.5 shrink-0">🚨</div>
          <div class="flex-1 min-w-0">
            <div class="font-bold text-danger">{{ s.nama }}</div>
            <div class="text-xs text-ink-2 mt-0.5">
              {{ s.kelas }} · Screen time: {{ s.screen_time }}j/hr · Konsentrasi: {{ s.fokus }}% · Risk score: {{ s.risk }}
            </div>
            <div class="text-xs text-ink-3 mt-1">Screen time tinggi · Konsentrasi rendah · Segera lakukan konseling</div>
          </div>
          <button class="btn btn-secondary btn-sm shrink-0" @click="$emit('open-detail', s)">Detail</button>
        </div>
      </div>
    </div>

    <!-- Berisiko -->
    <div class="card">
      <div class="card-title">Berisiko — Perlu Dipantau</div>
      <div class="card-sub">Siswa dengan indikasi awal brain rot</div>
      <div v-if="!berisiko.length" class="text-center py-6">
        <div class="text-3xl mb-2">👍</div>
        <div class="text-sm font-semibold text-ink-2">Tidak ada siswa berisiko</div>
      </div>
      <div v-else class="flex flex-col gap-2.5">
        <div v-for="s in berisiko" :key="s.nama"
          class="flex items-start gap-3.5 p-4 rounded-lg border bg-warn-bg border-warn-mid">
          <div class="text-lg mt-0.5 shrink-0">⚠️</div>
          <div class="flex-1 min-w-0">
            <div class="font-bold text-warn">{{ s.nama }}</div>
            <div class="text-xs text-ink-2 mt-0.5">
              {{ s.kelas }} · Screen time: {{ s.screen_time }}j/hr · Konsentrasi: {{ s.fokus }}% · Risk score: {{ s.risk }}
            </div>
            <div class="text-xs text-ink-3 mt-1">Pantau perkembangan dan berikan edukasi manajemen screen time</div>
          </div>
          <button class="btn btn-secondary btn-sm shrink-0" @click="$emit('open-detail', s)">Detail</button>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue'
const props = defineProps({ students: Array })
defineEmits(['open-detail', 'toast'])
const kritis = computed(() => props.students.filter(s => s.cluster === 'Kritis').sort((a, b) => b.risk - a.risk))
const berisiko = computed(() => props.students.filter(s => s.cluster === 'Berisiko').sort((a, b) => b.risk - a.risk))
</script>
